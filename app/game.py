...
import level_gen
import random
from enemies import DefaultEnemy, RocketEnemy
...
def map_constructor(level):
    ...
    for row in range(level.size[1]):
        for column in range(level.size[0]):
            if level.map[row][column] in ['B', '@', '%', '!', '8', '$', '&', '9']:
...
def Main(screen, res, clock, player, sound_on):
    ...
    vending_price = (player.levels // 2 * 1000) + 1000
    if vending_price > 8000:
            vending_price = 8000

    scale_ratio = (display.get_size()[0] / res[0], display.get_size()[1] / res[1])

    level = level_gen.Level((20, 20), 5 + player.levels, 10, 6 + player.levels)
    ...
    if level.vending_coords:
        vending_machine = SpecialTile('textures/vendingmachine.png', level.vending_coords, (125, 125), f'vending-{vending_price}')
    else:
    ...
    if vending_machine:
        interactables.append(vending_machine)
    ...
    if vending_machine:
        ...
        vending_font = pygame.font.Font('textures/upheavtt.ttf', 20)
        vending_text = vending_font.render(f"${vending_price}", True, (255, 255, 255))
        vending_rect = vending_text.get_rect()
        vending_rect.center = (vending_machine.img.get_width() / 2, int(vending_machine.img.get_height() / 5))
        

        vending_machine.img.blit(vending_text, vending_rect)
    ...
    while True:
        if not paused:
                    ...
                    if event.button == 1:
                        ...
                        if not player.dying:
                            ...
                            if removed_interactable != None:
                                ...
                                if removed_interactable.type == "used-vending":
                                    vending_machine.reset_img('textures/vendingmachine_empty.png', (125, 125))
                                elif not "vending-" in str(removed_interactable.type):
            ...
            blittable_blocks = [block for block in blocks if block.rect.topleft[0] in range(viewport_range[0][0], viewport_range[0][1]) and block.rect.topleft[1] in range(viewport_range[1][0], viewport_range[1][1])]

            blittable_enemies = [enemy for enemy in enemies if enemy.rect.topleft[0] in range(viewport_range[0][0], viewport_range[0][1]) and enemy.rect.topleft[1] in range(viewport_range[1][0], viewport_range[1][1]) and enemy.alive]

            blittable_boxes = [box for box in boxes if box.rect.topleft[0] in range(viewport_range[0][0], viewport_range[0][1]) and box.rect.topleft[1] in range(viewport_range[1][0], viewport_range[1][1])]
            ...
            if not player.jetpack:
                if not player.falling:
                    if player.rect.collidelist(blittable_blocks) == -1:
                        player.falling = True
